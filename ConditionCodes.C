...
//Jaracah Teague and Chris Caggia
                                      ...
                                      bool & error)
{
   ...
   switch(ccNum)
   {
    case OF:
        error = false;
        if(value) codes = Tools::setBits(codes, OF, OF);
        else codes = Tools::clearBits(codes, OF, OF);
        break;
    case SF:
        if(value) codes = Tools::setBits(codes, SF, SF);
        else codes = Tools::clearBits(codes, SF, SF);
        error = false;
        break;
    case ZF:
        if(value) codes = Tools::setBits(codes, ZF, ZF);
        else codes = Tools::clearBits(codes, ZF, ZF);
        error = false;
        break;   
    default: error = true;
   }
