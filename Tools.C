...
uint64_t Tools::getByte(uint64_t source, int32_t byteNum)
{
   ...
   source = (source >> ((4*byteNum))); 
   source = (source >> ((4*byteNum))) & 0xFF;
   return source;
...
{
   if ((srclow >= 0 && srclow <= 64) && (dstlow >= 0 && dstlow+length <= 64)) {
    uint64_t ret;
    uint64_t p1 = getBits(source, srclow, (srclow + (length - 1)));
    uint64_t p2 = clearBits(dest, dstlow, (dstlow + (length - 1)));
    p1 = p1 << (dstlow);
    ret = p1 | p2;
    return ret;
   } else {
...
{
   //works
   //source = setBits(source, (byteNum*8), ((byteNum*8)+7));  
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
   ...
   if ((sign(op1) == sign(op2)) && (sign(op1) == !sign(res))) {
       return 1;
   }
   return 0;
...
bool Tools::subOverflow(uint64_t op1, uint64_t op2)
{
   ...
   return ((sign(op1) != sign(op2)) && (sign(op2) == !sign(res)));
}
